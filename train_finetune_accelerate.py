...
from Modules.PLBERT.util import load_plbert
...
def main(config_path):
    ...
    for module in ["decoder"]:
        for g in optimizer.optimizers[module].param_groups:
            ...
            g['min_lr'] = 0
            g['weight_decay'] = 1e-4
    for module in [ "style_encoder","predictor_encoder"]:
        for g in optimizer.optimizers[module].param_groups:
            g['betas'] = (0.0, 0.99)
            g['lr'] = 1e-5
            g['initial_lr'] = 1e-5
    ...
    if load_pretrained:
        model, optimizer, start_epoch, iters = load_checkpoint(model, optimizer, config['pretrained_model'],
                                                               load_only_params=config.get('load_only_params', True),ignore_modules=[],if_pretrain=False)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch + 1) % save_freq == 0:
            ...
            save_config = osp.join(log_dir, osp.basename(config_path))
            ...
            save_config_hug = save_config.replace(".yml",f"_{epoch+1}.yml")

            api.upload_file(
                path_or_fileobj=save_config,
                path_in_repo=save_config_hug,
                repo_id="namkuner2402/Fonos_Dataset",
                repo_type="model",
            )
