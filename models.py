...
def load_state_dict_partial(model, state_dict):
    ...
    # Lọc ra những keys mà:
    # - Key có tồn tại trong model
    # - Shape phải khớp
    filtered_dict = {}
    for k, v in state_dict.items():
        if k in model_dict:
            if model_dict[k].shape == v.shape:
                ...
                print(
                    f"Skip loading key '{k}' due to shape mismatch: model {model_dict[k].shape} vs checkpoint {v.shape}")
        else:
            print(f"Skip loading key '{k}' because it's not in the model.")

    # Update model_dict với các weights hợp lệ
    model_dict.update(filtered_dict)
    model.load_state_dict(model_dict, strict=False)


def load_checkpoint(model, optimizer, path, load_only_params=True, ignore_modules=[], if_pretrain=False):
    ...
    for key in model:
        if key in params and key not in ignore_modules:
            print('%s loaded' % key)
            load_state_dict_partial(model[key], params[key])
        else:
            print("Không có key này", key)
